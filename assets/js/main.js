...
/*EJERCICIO 2*/
const cancionesEnMayusculas = songs.map((song) => 
song.title.toUpperCase()
);

console.log("Ex 2:", cancionesEnMayusculas);

/*EJERCICIO 3*/
const cancionesAntesDe1975 = songs.filter(song => 
    song.year < 1975);

console.log("Ex 3:", cancionesAntesDe1975);

/*EJERCICIO 4*/
const [primerTitulo] = songs;
console.log("Ex 4:", primerTitulo.title);

/*EJERCICIO 5*/
const hotelCalifornia = songs.find(song => 
...
/*function sumar(...argumento) {
    return argumento.reduce((total, current) => 
    total + current, 0);
  }
  
console.log("Ex 6:", sumar(10, 20, 30));
*/

/*EJERCICIO 7*/
const cancionesFormateadas = songs.map(song => 
    `${song.title} - ${song.artist} (${song.year})`
    );

console.log("Ex 7:", cancionesFormateadas);

/*EJERCICIO 8*/
const cancionesTheBeatles = songs.filter(({ artist }) => 
artist === 'The Beatles').map(({ title }) => title);

console.log("Ex 8:", cancionesTheBeatles);

/*EJERCICIO 9*/
const totalAnios = songs.reduce((acumulador, { year }) => 
acumulador + year, 0);

console.log("Ex 9:", totalAnios);

/*EJERCICIO 10*/
import {CalcularPromedioDeAnios} from "./CalcularPromedio.js";
const AveriguarAnio = CalcularPromedioDeAnios(songs);

console.log(`Ex 10: El año promedio de lanzamiento de las canciones es: ${AveriguarAnio}`);
